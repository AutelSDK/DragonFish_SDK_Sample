...
import com.autel.sdksample.base.mission.adapter.EvoWaypointFinishActionAdapter;
...
public class DFWaypointFragment extends WaypointMissionFragment {
    ...
    private CruiserWaypointFinishedAction finishedAction = CruiserWaypointFinishedAction.UNKNOWN;
    List<CruiserWaypoint> wayPointList = new ArrayList<>();

    @SuppressLint("ValidFragment")
    public DFWaypointFragment(MapOperator mMapOperator) {
        super(mMapOperator);
    }

    public DFWaypointFragment() {
        super();
    }
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        finishActionSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                finishedAction = (CruiserWaypointFinishedAction) parent.getAdapter().getItem(position);
            }
    ...
    public AutelMission createAutelMission() {
        CruiserWaypointMission waypointMission = new CruiserWaypointMission();
        waypointMission.missionId = BytesUtils.getInt(UUID.randomUUID().toString().replace("-", "").getBytes());
    ...
    protected void waypointAdded(AutelLatLng latLng) {
        ...
        CruiserWaypoint waypoint = new CruiserWaypoint(new AutelCoordinate3D(latLng.latitude, latLng.longitude, isEmpty(wHeight) ? 0 : Double.valueOf(wHeight)));
